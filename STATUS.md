...
- [x] Ajuste UX do `/motoboy` concluido localmente: fila e corrida ativa exibem o label `Loja solicitante` junto ao nome da loja; enderecos vazios, nulos ou whitespace nao renderizam linha, placeholder, alerta ou `Abrir no mapa`. Sem backend, contrato API, SQL/migration/RLS/grants/policies, PII nova, polling/realtime/push/cron/GPS/Storage/cancelamento/historico. Validacoes: `typecheck`, `test` (46), `lint`, `build` e `git diff --check` passaram.
