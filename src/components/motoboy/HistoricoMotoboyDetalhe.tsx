...
import { ClientApiError, getCourierHistoryDelivery } from '@/lib/api';
import { cn } from '@/lib/cn';
import type { CourierDeliveryHistoryDetail, DeliveryRequestStatus } from '@/types/delivery';

interface HistoricoMotoboyDetalheProps {
  accessToken: string;
  deliveryId: string;
}
  ...
  switch (error.code) {
    case 'DELIVERY_NOT_FOUND':
      return {
        ...
        message: 'Ela pode ter sido removida, ou não pertence ao seu histórico.',
      };
    case 'USER_PENDING':
      return {
        kind: 'recoverable',
    ...
    case 'COURIER_PROFILE_REQUIRED':
      return {
        ...
        message: 'Somente um motoboy ativo pode abrir esta corrida.',
      };
    case 'AUTH_REQUIRED':
    case 'INVALID_TOKEN':
...
function buildTimeline(delivery: CourierDeliveryHistoryDetail) {
  const steps = [
    {
      key: 'created',
      label: 'Criada',
...
export function HistoricoMotoboyDetalhe({
  accessToken,
  deliveryId,
}: HistoricoMotoboyDetalheProps) {
  const [delivery, setDelivery] = useState<CourierDeliveryHistoryDetail | null>(null);
  ...
  const load = useCallback(
    async (mode: 'initial' | 'refresh' = 'initial') => {
      ...
      try {
        const data = await getCourierHistoryDelivery(accessToken, deliveryId);
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        eyebrow="Histórico"
        title={`Corrida #${deliveryId.slice(0, 8)}`}
        description="Detalhe real de uma corrida atribuída ao seu perfil."
        actions={
          <ButtonLink href="/motoboy/historico" variant="secondary" size="md">
      ...
      {isLoading ? (
        <Card variant="paper" className="text-center">
          <div className="flex flex-col items-center gap-3 py-12 text-asphalt-950/70">
            ...
            <p className="text-sm font-semibold">Carregando corrida...</p>
          </div>
        </Card>
        ...
        <Card variant="white" className="text-center">
          <div className="flex flex-col items-center gap-4 py-10">
            ...
            <div className="flex flex-col gap-2 sm:flex-row">
              ...
              <ButtonLink href="/motoboy/historico" variant="primary" size="md">
                Ver histórico
              </ButtonLink>
            </div>
          </div>
        ...
        <section className="space-y-5" aria-live="polite">
          <Card variant="white" className="space-y-5">
            ...
            <div className="grid gap-3 md:grid-cols-2">
              <DetailTile icon={Store} label="Loja" value={delivery.store.name} />
              <DetailTile icon={MapPin} label="Coleta" value={delivery.store.address} />
              <DetailTile
                icon={MapPin}
                label="Destino"
